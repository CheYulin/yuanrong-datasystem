...
private:
     ...
     * @return Status of the call.
     */
    Status RunKeepAliveTask(Timer &keepAliveTimeoutTimer, int &networkFailedConfirmTimes, bool &localRecoveryPending);
     ...
     * @param[in,out] localRecoveryPending Whether local isolation was published and awaits a successful renewal.
     * @return True when the lease-expiry threshold was reached and the caller should delay before retrying.
     */
    bool ProcessKeepAliveFailure(const Status &status, int &confirmTimes, bool &localRecoveryPending);
