...
Status ZmqService::WorkerCB::WorkerEntry()
{
    ...
    GetLapTime(meta, TICK_SERVER_DEQUEUE);
    VLOG(RPC_LOG_LEVEL) << FormatString("Worker %s started for service '%s' Method %d serving %s", GetWorkerId(),
    ...
    GetLapTime(meta, TICK_SERVER_EXEC_END);
...
Status ZmqService::ServiceToClient(ZmqMetaMsgFrames &p)
{
    ...
    GetLapTime(meta, TICK_SERVER_SEND);
    ...
    // Record SERVER_SEND tick and calculate server-side metrics AFTER all reply data is queued.
    // TICK_SERVER_SEND marks when the reply meta+frames were handed to the transport layer;
    // SERVER_REPLY = SERVER_SEND - SERVER_EXEC_END captures the complete reply preparation cost
    // (serialization + zero-copy payload park + transport enqueue).
...
Status ZmqService::FrontendToBackend(int fd, const EventType type, ZmqMetaMsgFrames &p, bool addRoute)
{
    ...
    MetaPb &meta = p.first;
    GetLapTime(meta, TICK_SERVER_RECV);
