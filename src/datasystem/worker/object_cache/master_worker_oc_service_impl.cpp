...
Status MasterWorkerOCServiceImpl::PromotePrimaryCopy(const std::string &objectKey,
                                                     worker::WorkerRuntimeFacade::AdmissionGuard &admissionGuard,
                                                     bool &admissionRejected)
{
    admissionRejected = false;
    LOG(INFO) << FormatString("PromotePrimaryCopy for object:%s", objectKey);
    std::shared_ptr<SafeObjType> safeEntry;
    RETURN_IF_NOT_OK(ocClientWorkerSvc_->objectTable_->Get(objectKey, safeEntry));
    INJECT_POINT("process.promote.primary.copy.before.wlock");
    RETURN_IF_NOT_OK(safeEntry->WLock());
    Raii unlockRaii([safeEntry]() { safeEntry->WUnlock(); });
    if ((*safeEntry)->IsBinary()) {
        CHECK_FAIL_RETURN_STATUS(!(*safeEntry)->stateInfo.IsCacheInvalid(), K_NOT_READY,
                                 "cache-invalid copy cannot become primary");
    }

    if (runtime_ != nullptr) {
        auto admissionRc = runtime_->AcquireAdmissionGuard(worker::WorkerAdmissionKind::PRIMARY_PROMOTION_RPC,
                                                           "ChangePrimaryCopy", admissionGuard);
        if (admissionRc.IsError()) {
    ...
    if ((*safeEntry)->IsBinary()) {
        (*safeEntry)->stateInfo.SetPrimaryCopy(true);
    }
    INJECT_POINT("process.change.primary.copy");
    return Status::OK();
}
...
Status MasterWorkerOCServiceImpl::ChangePrimaryCopy(const ChangePrimaryCopyReqPb &req, ChangePrimaryCopyRspPb &rsp)
{
    ...
    for (const auto &id : req.object_keys()) {
        ...
        Status rc = PromotePrimaryCopy(id, admissionGuard, admissionRejected);
        if (admissionRejected) {
            if (rsp.success_ids().empty()) {
                return rc;
            }
            LOG(INFO) << "worker ChangePrimaryCopy stopped after partial success: " << rc.GetMsg();
            return Status::OK();
        }
        if (rc.IsError()) {
