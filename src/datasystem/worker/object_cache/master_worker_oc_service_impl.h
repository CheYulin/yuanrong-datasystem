...
private:
    ...
    /**
     * @brief Promote an object while preserving admission rejection separately from object-level failures.
     * @param[in] objectKey Object key to promote.
     * @param[out] admissionGuard Guard held from the first state mutation through success accounting.
     * @param[out] admissionRejected Whether runtime admission rejected the promotion.
     * @return Status of the promotion.
     */
    Status PromotePrimaryCopy(const std::string &objectKey, worker::WorkerRuntimeFacade::AdmissionGuard &admissionGuard,
