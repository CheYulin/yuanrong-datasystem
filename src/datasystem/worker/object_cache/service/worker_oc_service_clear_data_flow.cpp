...
constexpr uint64_t CLEAR_DATA_MAX_RETRY_ROUNDS = 10;
...
WorkerOcServiceClearDataFlow::~WorkerOcServiceClearDataFlow()
{
    ...
    auto retryLifetime = asyncRetryLifetime_;
    if (retryLifetime != nullptr) {
        std::unique_lock<std::mutex> lock(retryLifetime->mutex);
        retryLifetime->stopping = true;
        retryLifetime->cv.notify_all();
        retryLifetime->cv.wait(lock, [&retryLifetime] { return retryLifetime->activeCallbacks == 0; });
    }
...
void WorkerOcServiceClearDataFlow::SubmitOwnedTopologyCleanup(OwnedTopologyClearRequest request)
{
    ...
    clearDataThreadPool_->Execute([this, request = std::move(request), exitFlag] {
        ...
        if (!retryIds.Empty()) {
            ...
            RetryClearDataAsync({}, retryIds, 0);
        }
    });
}

void WorkerOcServiceClearDataFlow::SubmitClearDataAsync(const ClearDataReqPb &req, uint64_t completedRetryRounds)
{
    if (clearDataThreadPool_ == nullptr) {
    ...
    auto exitFlag = exitFlag_;
    clearDataThreadPool_->Execute([this, req, completedRetryRounds, traceID, exitFlag] {
        if (exitFlag != nullptr && exitFlag->load()) {
            return;
        }
        ...
        if (!retryIds.Empty()) {
            LOG(WARNING) << "ClearData async task need retry, completed retry rounds: " << completedRetryRounds
                         << ", failed object size: " << retryIds.Size() << ", status: " << rc.ToString();
            RetryClearDataAsync(req, retryIds, completedRetryRounds);
            return;
        }
...
void WorkerOcServiceClearDataFlow::SubmitRetryClearDataAsync(const ClearDataReqPb &req, uint64_t retryRound,
                                                             const ClearDataRetryIds &retryIds)
{
    ...
    auto traceID = Trace::Instance().GetTraceID();
    auto exitFlag = exitFlag_;
    clearDataThreadPool_->Execute([this, req, retryRound, retryIds, traceID, exitFlag] {
        if (exitFlag != nullptr && exitFlag->load()) {
            return;
        }
        TraceGuard traceGuard = Trace::Instance().SetTraceNewID(traceID);
        ...
        if (!nextRetryIds.Empty()) {
            LOG(WARNING) << "Retry ClearData async task need retry, retry round: " << retryRound
                         << ", failed object size: " << nextRetryIds.Size();
            RetryClearDataAsync(req, nextRetryIds, retryRound);
            return;
        }
    });
}
...
void WorkerOcServiceClearDataFlow::RetryClearDataAsync(const ClearDataReqPb &req, const ClearDataRetryIds &retryIds,
                                                       uint64_t completedRetryRounds)
{
    if (exitFlag_ != nullptr && exitFlag_->load()) {
        return;
    }
    if (completedRetryRounds >= CLEAR_DATA_MAX_RETRY_ROUNDS) {
        LOG(ERROR) << "ClearData retry budget exhausted, completed retry rounds: " << completedRetryRounds
                   << ", exhausted stage entry count: " << retryIds.Size()
                   << ", clear failed count: " << retryIds.clearFailedIds.size()
                   << ", increase failed count: " << retryIds.increaseFailedIds.size()
                   << ", recover app ref failed count: " << retryIds.recoverAppRefFailedIds.size();
        return;
    }
    ...
    auto exitFlag = exitFlag_;
    auto retryLifetime = asyncRetryLifetime_;
    const uint64_t retryRound = completedRetryRounds + 1;
    auto retryTask = [this, req, retryIds, retryRound, traceID, exitFlag, retryLifetime] {
        std::unique_lock<std::mutex> lock(retryLifetime->mutex);
        if (retryLifetime->stopping) {
            return;
        }
        Raii activeCallback([retryLifetime] {
            std::lock_guard<std::mutex> guard(retryLifetime->mutex);
            --retryLifetime->activeCallbacks;
            if (retryLifetime->activeCallbacks == 0) {
                retryLifetime->cv.notify_all();
            }
        });
        ++retryLifetime->activeCallbacks;
        ...
        TraceGuard traceGuard = Trace::Instance().SetTraceNewID(traceID);
        SubmitRetryClearDataAsync(req, retryRound, retryIds);
    };
