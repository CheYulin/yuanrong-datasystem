...
public:
    ...
    using FinishFn = std::function<Status(const cluster::MemberIdentity &, int64_t, RpcRecvFlags,
                                          cluster::ControlBackendObservation &)>;
    using ForgetFn = std::function<void(int64_t)>;

    WorkerControlBackendProbe(StartFn start, FinishFn finish, ForgetFn forget);
    ~WorkerControlBackendProbe() = default;

    Status Start(int32_t timeoutMs, int64_t &tag);

    Status Finish(const cluster::MemberIdentity &peer, int64_t tag, RpcRecvFlags flags,
                  cluster::ControlBackendObservation &observation);

    Status Forget(int64_t tag);
...
using WorkerControlBackendProbeFactory =
    std::function<Status(const cluster::MemberIdentity &, std::chrono::steady_clock::time_point,
                         std::unique_ptr<WorkerControlBackendProbe> &)>;
