...
void WorkerIsolationCoordinator::OnLocalIsolation(const Status &status)
{
    LOG(WARNING) << "Close worker business admission after confirmed local control-backend isolation: "
                 << status.ToString();
    runtime_.MarkLocalIsolated(
        WorkerIsolationReason::CONTROL_BACKEND_LOCAL_ISOLATION, status.ToString(),
        [this](const WorkerAdmissionGateUpdate &update) { SetTopologyServingAdmission(update); });
...
void WorkerIsolationCoordinator::OnLocalRecovery()
{
    ...
    LOG_IF_ERROR(hooks_.requestRecoveryReconciliation([this] {
        runtime_.MarkRecovering(
            WorkerIsolationReason::RECOVERY_EVIDENCE_INCOMPLETE,
            "control backend keepalive renewal recovered; reconciling topology", WorkerRecoveryPhase::TOPOLOGY,
            [this](const WorkerAdmissionGateUpdate &update) { SetTopologyServingAdmission(update); });
    }),
                 "Queue authoritative topology reconciliation after control backend recovery");
}

void WorkerIsolationCoordinator::SetTopologyServingAdmission(const WorkerAdmissionGateUpdate &update) const
{
    if (hooks_.setTopologyServingAdmission != nullptr) {
        hooks_.setTopologyServingAdmission(update);
    }
}
}  // namespace datasystem::worker
