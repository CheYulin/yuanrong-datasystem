...
{
    const auto recovery = PrepareRecovery(evidence, detail);
    const bool recovered = runtimeState_.TryMarkRunning(recovery.evidence, recovery.detail);
    FinishRecovery();
    return recovered;
}

std::optional<bool> WorkerRecoveryController::TryCompleteRecovery(WorkerRecoveryGeneration generation,
                                                                  const WorkerRunningEvidence &evidence,
                                                                  const std::string &detail)
{
    const auto recovery = PrepareRecovery(evidence, detail);
    const auto recovered = CommitPreparedRecovery(generation, recovery);
    FinishRecovery();
    return recovered;
}

WorkerRecoveryController::PreparedRecovery WorkerRecoveryController::PrepareRecovery(
    const WorkerRunningEvidence &evidence, const std::string &detail) const
{
    INJECT_POINT_NO_RETURN("WorkerRecoveryController.BeforeMarkRunning");
    return PreparedRecovery{ evidence, WithMissingEvidenceDetail(evidence, detail) };
}

std::optional<bool> WorkerRecoveryController::CommitPreparedRecovery(WorkerRecoveryGeneration generation,
                                                                     const PreparedRecovery &recovery)
{
    return runtimeState_.TryMarkRunning(generation, recovery.evidence, recovery.detail);
}

void WorkerRecoveryController::FinishRecovery() const
{
