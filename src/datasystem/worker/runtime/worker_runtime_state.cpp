...
bool WorkerRuntimeStateManager::BeginRecoveryEvidenceGeneration(WorkerRecoveryGeneration generation, std::string detail)
{
    auto admissionLock = AcquireTransitionGuard();
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    recoveryEvidenceGeneration_ = generation;
    if (IsServingTransitionTerminalLocked()) {
        return true;
    }
    ...
    if (snapshot_.mode == WorkerServiceMode::RUNNING) {
        (void)UpdateLocked(WorkerServiceMode::RUNNING, WorkerIsolationReason::NONE, evidence,
                           WorkerRecoveryPhase::METADATA, std::move(detail));
    } else if (snapshot_.mode == WorkerServiceMode::LOCAL_ISOLATED
               || snapshot_.mode == WorkerServiceMode::OUT_OF_MEMORY) {
        (void)UpdateLocked(snapshot_.mode, snapshot_.reason, evidence, WorkerRecoveryPhase::METADATA,
                           std::move(detail));
    } else {
...
bool WorkerRuntimeStateManager::TryMarkRunning(const WorkerRunningEvidence &evidence, std::string detail)
{
    auto admissionLock = AcquireTransitionGuard();
    std::lock_guard<std::mutex> lock(mutex_);
    return TryMarkRunningLocked(evidence, std::move(detail));
}

std::optional<bool> WorkerRuntimeStateManager::TryMarkRunning(WorkerRecoveryGeneration generation,
                                                              const WorkerRunningEvidence &evidence, std::string detail)
{
    auto admissionLock = AcquireTransitionGuard();
    std::lock_guard<std::mutex> lock(mutex_);
    if (generation == 0 || generation != recoveryEvidenceGeneration_) {
...
bool WorkerRuntimeStateManager::TryMarkRunningLocked(const WorkerRunningEvidence &evidence, std::string detail)
{
    bool running = false;
    if (IsServingTransitionTerminalLocked()) {
        return false;
    ...
    if (!IsServingReady(evidence)) {
        ...
        if (snapshot_.mode == WorkerServiceMode::RUNNING && IsComplete(snapshot_.evidence) && !IsComplete(evidence)) {
            return false;
        }
        (void)UpdateLocked(WorkerServiceMode::RUNNING, WorkerIsolationReason::NONE, evidence,
                           IsComplete(evidence) ? WorkerRecoveryPhase::COMPLETE : FirstMissingRecoveryPhase(evidence),
                           std::move(detail));
        running = true;
    }
    return running;
