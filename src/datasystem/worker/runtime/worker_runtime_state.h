...
public:
    ...
    std::optional<bool> TryMarkRunning(WorkerRecoveryGeneration generation, const WorkerRunningEvidence &evidence,
                                       std::string detail = {});
...
private:
    ...
    bool TryMarkRunningLocked(const WorkerRunningEvidence &evidence, std::string detail);
    ...
    WorkerRecoveryGeneration recoveryEvidenceGeneration_{ 1 };
    WorkerRuntimeStateSnapshot snapshot_;
};
}  // namespace datasystem::worker
