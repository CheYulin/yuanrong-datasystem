...
bool RequiresCompleteReconciliation(WorkerAdmissionKind kind)
{
    return kind == WorkerAdmissionKind::MIGRATION_TARGET;
}
...
Status Evaluate(const WorkerRuntimeStateSnapshot &snapshot, WorkerAdmissionKind kind, const std::string &operation)
{
    switch (snapshot.mode) {
        case WorkerServiceMode::RUNNING:
            if (RequiresCompleteReconciliation(kind) && !IsComplete(snapshot.evidence)) {
                return Reject(snapshot, kind, operation, K_NOT_READY, "RECONCILIATION_INCOMPLETE");
            }
            return Status::OK();
...
const char *ToString(WorkerAdmissionKind kind)
{
    switch (kind) {
        ...
        case WorkerAdmissionKind::PRIMARY_PROMOTION_RPC:
            return "PRIMARY_PROMOTION_RPC";
...
{
    if (!RequiresCompleteReconciliation(kind) && runtimeState_.IsFastRunningForAdmission()) {
        return Status::OK();
    }
