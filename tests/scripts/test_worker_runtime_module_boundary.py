...
class WorkerRuntimeModuleBoundaryTest(unittest.TestCase):
    def test_control_backend_scope_classifier_does_not_depend_on_object_cache_transport(self):
        build_files = {
            REPO_ROOT / "src/datasystem/cluster/BUILD.bazel",
            REPO_ROOT / "src/datasystem/cluster/CMakeLists.txt",
        }
        ...
        for build_file in build_files:
            ...
            if build_file.name == "BUILD.bazel":
                text = text.split('name = "cluster_control_backend_scope_classifier"', 1)[1]
                text = text.split("ds_cc_library(", 1)[0]
            else:
                text = text.split("add_library(cluster_control_backend_scope_classifier", 1)[1]
                text = text.split("add_library(", 1)[0]
            for token in forbidden_tokens:
                self.assertNotIn(token, text, f"{build_file} should not expose object_cache dependencies")

    def test_control_backend_scope_classifier_is_cluster_owned(self):
        cluster_files = [
            REPO_ROOT / "src/datasystem/cluster/runtime/control_backend_scope_classifier.h",
            REPO_ROOT / "src/datasystem/cluster/runtime/control_backend_scope_classifier.cpp",
        ]
        for file_path in cluster_files:
            self.assertTrue(file_path.exists(), f"{file_path} should own control-backend scope classification")

        forbidden_paths = [
            REPO_ROOT / "src/datasystem/worker/runtime/worker_control_backend_scope.h",
            REPO_ROOT / "src/datasystem/worker/runtime/worker_control_backend_scope_classification.cpp",
        ]
        for file_path in forbidden_paths:
            self.assertFalse(file_path.exists(), f"{file_path} should not expose cluster scope policy from worker")

        worker_build_files = [
            REPO_ROOT / "src/datasystem/worker/runtime/BUILD.bazel",
            REPO_ROOT / "src/datasystem/worker/runtime/CMakeLists.txt",
        ]
        for file_path in worker_build_files:
            self.assertNotIn("worker_control_backend_scope", file_path.read_text(encoding="utf-8"))
    ...
    def test_data_migrator_does_not_depend_on_topology_callback_executor(self):
        ...
        forbidden_tokens = [
            "datasystem/cluster/executor/topology_phase_callbacks.h",
            "datasystem/worker/runtime/worker_topology_runtime.h",
        ]
    ...
    def test_object_cache_async_paths_do_not_capture_runtime_admission_guard(self):
        source = REPO_ROOT / "src/datasystem/worker/object_cache/worker_oc_service_impl.cpp"
        text = source.read_text(encoding="utf-8")

        forbidden_tokens = [
            "AcquireObjectCacheAsyncAdmissionGuard",
            "std::shared_ptr<std::optional<RuntimeAdmissionGuard>>",
            "(void)admissionGuard;",
        ]
        for token in forbidden_tokens:
            self.assertFalse(token in text, f"{token} should not appear in async RPC admission paths")

    def test_stream_async_paths_do_not_capture_runtime_admission_guard(self):
        source = REPO_ROOT / "src/datasystem/worker/stream_cache/client_worker_sc_service_impl.cpp"
        text = source.read_text(encoding="utf-8")

        forbidden_tokens = [
            "AcquireStreamAsyncAdmissionGuard",
            "std::shared_ptr<std::optional<RuntimeAdmissionGuard>>",
            "(void)admissionGuard;",
        ]
        for token in forbidden_tokens:
    ...
    def test_worker_oc_server_delegates_concrete_coordination_backend_construction(self):
        ...
        self.assertIn("class EtcdStore;", factory_text)
        self.assertNotIn("datasystem/common/kvstore/etcd/etcd_store.h", factory_text)
    ...
    def test_topology_availability_thin_adapter_is_not_exposed(self):
        header = REPO_ROOT / "src/datasystem/worker/runtime/worker_topology_availability_admission.h"
        impl = REPO_ROOT / "src/datasystem/worker/runtime/worker_topology_availability_admission.cpp"
        self.assertFalse(header.exists(), "Topology availability should use WorkerRuntimeFacade directly")
        self.assertFalse(impl.exists(), "Topology availability should not keep a production-thin adapter target")
        for build_file in [
            REPO_ROOT / "src/datasystem/worker/runtime/BUILD.bazel",
            REPO_ROOT / "src/datasystem/worker/runtime/CMakeLists.txt",
        ]:
            self.assertNotIn("worker_topology_availability_admission", build_file.read_text(encoding="utf-8"))
    ...
    def test_runtime_recovery_covers_disabled_slot_passive_overlay(self):
        runtime_test = REPO_ROOT / "tests/ut/worker/worker_runtime_facade_topology_availability_test.cpp"
        self.assertIn(
