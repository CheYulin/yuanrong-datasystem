...
Status DirectWorkerHealthCheck(const HostPort &workerAddr, const RpcCredential &credential,
                               const HealthCheckRequestPb &req, HealthCheckReplyPb &rsp, int32_t timeoutMs)
{
    RpcOptions opts;
    opts.SetTimeout(timeoutMs);
    if (FLAGS_use_brpc) {
        ...
        CHECK_FAIL_RETURN_STATUS(channel != nullptr, K_RPC_UNAVAILABLE,
                                 "Failed to create brpc WorkerOCService channel to " + cfg.endpoint);
        WorkerOCService_BrpcGenericStub stub(channel.get(), timeoutMs);
        return stub.HealthCheck(opts, req, rsp);
#else
        (void)workerAddr;
        (void)credential;
        (void)req;
...
TEST_F(WorkerPushMetaScaleOutFaultTest, LEVEL1_ScaleOutSurvivesTransientGlobalBackendOutage)
{
    ...
    bool globalOutageActive[3]{};
    bool leaseOverrideActive[3]{};
    bool quickLoopActive[3]{};
    Raii clearGlobalBackendOutageInjects([&]() {
        for (uint32_t workerIndex = 0; workerIndex < 3; ++workerIndex) {
            if (globalOutageActive[workerIndex]) {
                LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, workerIndex, "WorkerOCServer.GlobalBackendOutage"),
                             "clear global-backend outage classification hook");
            }
            if (leaseOverrideActive[workerIndex]) {
                LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, workerIndex, "GetLeaseExpiredMs"),
                             "clear global-backend outage lease override");
            }
            if (quickLoopActive[workerIndex]) {
                LOG_IF_ERROR(
                    cluster_->ClearInjectAction(WORKER, workerIndex, "EtcdStore.LaunchKeepAliveThreads.loopQuickly"),
                    "clear global-backend outage quick keepalive loop");
            }
        }
    });
    for (uint32_t workerIndex = 0; workerIndex < 3; ++workerIndex) {
...
TEST_F(WorkerPushMetaTest, LEVEL1_TestKeepAliveLocalIsolationKeepsWorkerAliveAndProtectsPeerData)
{
    ...
    bool beforeValidateActive = false;
    bool createBeginActive = false;
    bool localIsolationHookActive = false;
    bool keepAliveFailureActive = false;
    bool leaseOverrideActive = false;
    bool quickLoopActive = false;
    Raii clearLocalIsolationInjects([&]() {
        if (beforeValidateActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "worker.Create.beforeValidate"),
                         "clear local-isolation admission probe");
        }
        if (createBeginActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "worker.Create.begin"),
                         "clear local-isolation allocation probe");
        }
        if (localIsolationHookActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "WorkerOCServer.AfterMarkLocalIsolated"),
                         "clear local-isolation barrier");
        }
        if (keepAliveFailureActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdKeepAlive.SendKeepAliveMessage"),
                         "clear local-isolation keepalive failure");
        }
        if (leaseOverrideActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "GetLeaseExpiredMs"),
                         "clear local-isolation lease override");
        }
        if (quickLoopActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdStore.LaunchKeepAliveThreads.loopQuickly"),
                         "clear local-isolation quick keepalive loop");
        }
    });
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "worker.Create.beforeValidate", "call()"));
    beforeValidateActive = true;
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "worker.Create.begin", "call()"));
    createBeginActive = true;
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "WorkerOCServer.AfterMarkLocalIsolated", "call()"));
    localIsolationHookActive = true;
    ...
    AssertEventuallyOk(
        [&]() {
            ...
            CHECK_FAIL_RETURN_STATUS(isolationCount > 0, K_NOT_READY,
                                     "worker has not entered confirmed local isolation");
            ...
            if (beforeValidateAfter > beforeValidateBefore && createBeginAfter == createBeginBefore) {
                ...
                CHECK_FAIL_RETURN_STATUS(createStatus.GetMsg().find("phase=METADATA") != std::string::npos, K_NOT_READY,
...
TEST_F(WorkerPushMetaTest, LEVEL1_TestPeerWorkerKvDataSurvivesLocalIsolationAndRecovery)
{
    ...
    bool isolationCandidateActive = false;
    bool keepAliveFailureActive = false;
    bool leaseOverrideActive = false;
    bool quickLoopActive = false;
    Raii clearKvIsolationInjects([&]() {
        if (isolationCandidateActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "WorkerOCServer.LocalBackendIsolationCandidate"),
                         "clear KV local-isolation candidate hook");
        }
        if (keepAliveFailureActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdKeepAlive.SendKeepAliveMessage"),
                         "clear KV local-isolation keepalive failure");
        }
        if (leaseOverrideActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "GetLeaseExpiredMs"),
                         "clear KV local-isolation lease override");
        }
        if (quickLoopActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdStore.LaunchKeepAliveThreads.loopQuickly"),
                         "clear KV local-isolation quick keepalive loop");
        }
    });
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "WorkerOCServer.LocalBackendIsolationCandidate", "call()"));
    isolationCandidateActive = true;
    DS_ASSERT_OK(
        cluster_->SetInjectAction(WORKER, 0, "EtcdKeepAlive.SendKeepAliveMessage", "return(K_RPC_UNAVAILABLE)"));
    keepAliveFailureActive = true;
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "GetLeaseExpiredMs", "call(1000)"));
...
TEST_F(WorkerStalePrimaryTest, LEVEL1_IsolatedWorkerMetaCleanupAllowsNewOwnerRebuild)
{
    ...
    ASSERT_TRUE(workerProcess->IsProcessAlive());
    const auto workerPid = workerProcess->Pid();
    bool peerPrimaryChangeActive = false;
    bool oldPrimaryChangeActive = false;
    bool localIsolationHookActive = false;
    bool keepAliveFailureActive = false;
    bool leaseOverrideActive = false;
    bool quickLoopActive = false;
    Raii clearStalePrimaryInjects([&]() {
        if (peerPrimaryChangeActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 1, "process.change.primary.copy"),
                         "clear peer primary-change hook");
        }
        if (oldPrimaryChangeActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "process.change.primary.copy"),
                         "clear old-primary change hook");
        }
        if (localIsolationHookActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "WorkerOCServer.AfterMarkLocalIsolated"),
                         "clear old-primary local-isolation hook");
        }
        if (keepAliveFailureActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdKeepAlive.SendKeepAliveMessage"),
                         "clear old-primary keepalive failure");
        }
        if (leaseOverrideActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "GetLeaseExpiredMs"),
                         "clear old-primary lease override");
        }
        if (quickLoopActive) {
            LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, "EtcdStore.LaunchKeepAliveThreads.loopQuickly"),
                         "clear old-primary quick keepalive loop");
        }
    });
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 1, "process.change.primary.copy", "call()"));
    peerPrimaryChangeActive = true;
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "process.change.primary.copy", "call()"));
    oldPrimaryChangeActive = true;
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, "WorkerOCServer.AfterMarkLocalIsolated", "call()"));
    localIsolationHookActive = true;
    DS_ASSERT_OK(
        cluster_->SetInjectAction(WORKER, 0, "EtcdKeepAlive.SendKeepAliveMessage", "return(K_RPC_UNAVAILABLE)"));
    ...
    localIsolationHookActive = false;
}
...
TEST_F(WorkerPushMetaTransportTest, LEVEL1_TestTransportReadRejectedDuringIsolationAndBeforeRunning)
{
    ...
    RpcCredential workerCredential;
    DS_ASSERT_OK(RpcAuthKeyManager::CreateClientCredentials(RpcAuthKeys(), WORKER_SERVER_NAME, workerCredential));
    auto signature = std::make_shared<Signature>(transportOptions.accessKey, transportOptions.secretKey);
    BrpcChannelConfig channelConfig;
    channelConfig.timeout_ms = transportOptions.requestTimeoutMs;
    channelConfig.connect_timeout_ms = transportOptions.requestTimeoutMs;
    channelConfig.enable_circuit_breaker = false;
    channelConfig.max_retry = 0;
    auto directRpcClient = std::make_shared<client::WorkerRpcClient>(
        HostPort(worker0Addr.Host(), worker0Addr.Port() + kBrpcPortOffset), std::move(signature), channelConfig);
    DS_ASSERT_OK(directRpcClient->Init());
    client::TcpTransporter worker0Transport(directRpcClient);
    client::DataGetBatchRequest directReadRequests;
    for (size_t i = 0; i < worker0Keys.size(); ++i) {
        directReadRequests.emplace_back(
            client::DataGetRequest{ .objectKey = worker0Keys[i], .expectedSize = preservedData[i].size() });
    }

    // Establish SDK-level availability separately; this routed read is not used as worker-0 admission evidence.
    std::vector<Optional<Buffer>> buffers;
    ...
    constexpr const char *isolatedPause = "WorkerOCServer.AfterMarkLocalIsolated";
    constexpr const char *beforeRunningPause = "WorkerRecoveryController.BeforeMarkRunning";
    constexpr const char *afterRunningPause = "WorkerRecoveryController.AfterTryMarkRunning";
    constexpr const char *localLookup = "worker.worker_worker_remote_get_failure";
    constexpr const char *keepAliveFailure = "EtcdKeepAlive.SendKeepAliveMessage";
    constexpr const char *leaseOverride = "GetLeaseExpiredMs";
    constexpr const char *quickKeepAliveLoop = "EtcdStore.LaunchKeepAliveThreads.loopQuickly";
    bool injectsActive = true;
    Raii clearInjects([&]() {
        if (!injectsActive) {
            return;
        }
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, isolatedPause), "clear transport isolation pause");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, beforeRunningPause), "clear transport pre-running pause");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, afterRunningPause), "clear transport post-running pause");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, localLookup), "clear transport local-lookup observer");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, keepAliveFailure), "clear transport keepalive failure");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, leaseOverride), "clear transport lease override");
        LOG_IF_ERROR(cluster_->ClearInjectAction(WORKER, 0, quickKeepAliveLoop), "clear transport quick loop");
    });

    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, isolatedPause, "pause"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, beforeRunningPause, "pause"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, afterRunningPause, "pause"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, localLookup, "call()"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, keepAliveFailure, "return(K_RPC_UNAVAILABLE)"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, leaseOverride, "call(1000)"));
    DS_ASSERT_OK(cluster_->SetInjectAction(WORKER, 0, quickKeepAliveLoop, "call(0)"));
    auto waitForInject = [&](const char *name, const std::string &detail) {
        AssertEventuallyOk(
            [&]() {
                uint64_t executeCount = 0;
                RETURN_IF_NOT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, name, executeCount));
                CHECK_FAIL_RETURN_STATUS(executeCount > 0, K_NOT_READY, detail);
                return Status::OK();
            },
            detail);
    };

    waitForInject(isolatedPause, "transport data owner has not reached the local-isolation admission barrier");
    uint64_t isolatedLookupCountBefore = 0;
    DS_ASSERT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, localLookup, isolatedLookupCountBefore));
    client::DataGetBatchResult isolatedResults;
    auto isolatedReadRc = worker0Transport.BatchGet(directReadRequests, isolatedResults);
    uint64_t isolatedLookupCountAfter = 0;
    DS_ASSERT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, localLookup, isolatedLookupCountAfter));
    ASSERT_EQ(isolatedReadRc.GetCode(), K_NOT_READY);
    ASSERT_NE(isolatedReadRc.GetMsg().find("Worker is not accepting BatchGetObjectRemote"), std::string::npos)
        << isolatedReadRc.ToString();
    ASSERT_NE(isolatedReadRc.GetMsg().find("kind=NORMAL_READ"), std::string::npos) << isolatedReadRc.ToString();
    ASSERT_EQ(isolatedLookupCountAfter, isolatedLookupCountBefore);

    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, isolatedPause));
    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, keepAliveFailure));
    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, leaseOverride));
    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, quickKeepAliveLoop));

    waitForInject(beforeRunningPause, "transport recovery has not reached the pre-running barrier");
    uint64_t recoveringLookupCountBefore = 0;
    DS_ASSERT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, localLookup, recoveringLookupCountBefore));
    client::DataGetBatchResult recoveringResults;
    auto recoveringReadRc = worker0Transport.BatchGet(directReadRequests, recoveringResults);
    uint64_t recoveringLookupCountAfter = 0;
    DS_ASSERT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, localLookup, recoveringLookupCountAfter));
    ASSERT_EQ(recoveringReadRc.GetCode(), K_NOT_READY);
    ASSERT_NE(recoveringReadRc.GetMsg().find("Worker is not accepting BatchGetObjectRemote"), std::string::npos)
        << recoveringReadRc.ToString();
    ASSERT_NE(recoveringReadRc.GetMsg().find("kind=NORMAL_READ"), std::string::npos) << recoveringReadRc.ToString();
    ASSERT_EQ(recoveringLookupCountAfter, recoveringLookupCountBefore);
    uint64_t afterRunningCountBefore = 0;
    DS_ASSERT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, afterRunningPause, afterRunningCountBefore));
    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, beforeRunningPause));

    AssertEventuallyOk(
        [&]() {
            uint64_t executeCount = 0;
            RETURN_IF_NOT_OK(cluster_->GetInjectActionExecuteCount(WORKER, 0, afterRunningPause, executeCount));
            CHECK_FAIL_RETURN_STATUS(executeCount > afterRunningCountBefore, K_NOT_READY,
                                     "transport recovery has not reached the post-running barrier");
            return Status::OK();
        },
        "transport recovery has not reached the post-running barrier");
    client::DataGetBatchResult recoveredResults;
    DS_ASSERT_OK(worker0Transport.BatchGet(directReadRequests, recoveredResults));
    ASSERT_EQ(recoveredResults.size(), preservedData.size());
    for (size_t i = 0; i < recoveredResults.size(); ++i) {
        DS_ASSERT_OK(recoveredResults[i].status);
        ASSERT_EQ(recoveredResults[i].data.rpcPayloads.size(), 1u);
        const auto &payload = recoveredResults[i].data.rpcPayloads.front();
        ASSERT_EQ(payload.Size(), preservedData[i].size());
        EXPECT_EQ(std::string(reinterpret_cast<const char *>(payload.Data()), payload.Size()), preservedData[i]);
    }
    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, afterRunningPause));
    HealthCheckRequestPb healthReq;
    HealthCheckReplyPb healthRsp;
    DS_ASSERT_OK(DirectWorkerHealthCheck(worker0Addr, workerCredential, healthReq, healthRsp,
                                         transportOptions.requestTimeoutMs));
    ASSERT_EQ(healthRsp.worker_service_mode(), "RUNNING");
    ...
    AssertBufferEqual(*buffers[0], preservedData[0]);
    AssertBufferEqual(*buffers[1], preservedData[1]);
    ASSERT_GT(cluster_->GetWorkerPid(0), 0);

    DS_ASSERT_OK(cluster_->ClearInjectAction(WORKER, 0, localLookup));
    injectsActive = false;
}
