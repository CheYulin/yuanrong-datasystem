...
TEST_F(StreamClientAdmissionTest, LEVEL1_StreamClientRejectsFourWriteRegisterEntriesDuringIsolationAndRecovering)
{
    ...
    std::shared_ptr<Producer> pageProducer;
    DS_ASSERT_OK(client_->CreateProducer(NewStreamName("stream_admission_page"), pageProducer, producerConf_));
    std::string pagePayload(static_cast<size_t>(producerConf_.pageSize / 2), 'p');
    DS_ASSERT_OK(pageProducer->Send(Element(reinterpret_cast<uint8_t *>(pagePayload.data()), pagePayload.size())));
    std::shared_ptr<Producer> bigElementProducer;
    DS_ASSERT_OK(client_->CreateProducer(NewStreamName("stream_admission_lob"), bigElementProducer, producerConf_));
    std::string bigElementPayload(static_cast<size_t>(producerConf_.pageSize * 2), 'b');

    auto expectMemoryEntriesRejected = [&](const std::string &phase) {
        ExpectAdmissionRejected(
            pageProducer->Send(Element(reinterpret_cast<uint8_t *>(pagePayload.data()), pagePayload.size()), 1'000),
            "stream CreateShmPage via normal Send during " + phase);
        ExpectAdmissionRejected(
            bigElementProducer->Send(
                Element(reinterpret_cast<uint8_t *>(bigElementPayload.data()), bigElementPayload.size()), 1'000),
            "stream AllocBigShmMemory via big Send during " + phase);
    };
    ...
    expectMemoryEntriesRejected("local isolation");
    ...
    expectMemoryEntriesRejected("recovery");
