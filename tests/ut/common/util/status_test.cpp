...
TEST_F(StatusTest, EventWaitForTimeoutReturnsUrmaWaitTimeout)
{
    auto event = std::make_shared<Event>(1);
    Status status = event->WaitFor(std::chrono::milliseconds(0));
    ASSERT_EQ(status.GetCode(), StatusCode::K_URMA_WAIT_TIMEOUT);
    ASSERT_TRUE(status.GetMsg().find("[URMA_WAIT_TIMEOUT]") != std::string::npos);
}
...
TEST_F(StatusTest, EventWaitAnyTimeoutReturnsUrmaWaitTimeout)
{
