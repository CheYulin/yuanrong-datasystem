...
TEST(WorkerRecoveryControllerTest, IncompleteReconciliationEvidenceOpensServing)
{
    ...
    EXPECT_TRUE(recovery.TryCompleteRecovery(evidence, "ownership unresolved"));
    ...
    EXPECT_EQ(snapshot.recoveryPhase, WorkerRecoveryPhase::OWNERSHIP);
    EXPECT_FALSE(snapshot.evidence.ownershipReady);
    EXPECT_NE(snapshot.detail.find("missing=ownership"), std::string::npos);
}

TEST(WorkerRecoveryControllerTest, NewGenerationKeepsServingButRejectsStaleCompletion)
{
    WorkerRuntimeStateManager state;
    WorkerRecoveryController recovery(state);
    ...
    ASSERT_TRUE(state.BeginRecoveryEvidenceGeneration(2, "g2 pending"));
    EXPECT_EQ(state.GetSnapshot().mode, WorkerServiceMode::RUNNING);
    EXPECT_TRUE(state.IsFastRunningForAdmission());
    EXPECT_FALSE(recovery.TryCompleteRecovery(1, CompleteEvidence(), "stale g1").has_value());
    EXPECT_FALSE(IsComplete(state.GetSnapshot().evidence));
}
