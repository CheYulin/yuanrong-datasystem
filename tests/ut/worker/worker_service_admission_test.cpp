...
TEST(WorkerServiceAdmissionTest, RunningSuccessPathDoesNotCopyFullSnapshotDetail)
{
    ...
    EXPECT_EQ(admission.Check(WorkerAdmissionKind::NORMAL_WRITE, "Create").GetCode(), K_NOT_READY);
    EXPECT_EQ(inject::GetExecuteCount("WorkerRuntimeState.GetSnapshot"), 2ul);
}

TEST(WorkerServiceAdmissionTest, RunningWithIncompleteReconciliationRejectsMigrationTargetOnly)
{
    WorkerRuntimeStateManager state;
    auto evidence = CompleteEvidence();
    evidence.slotReady = false;
    evidence.ownershipReady = false;
    ASSERT_TRUE(state.TryMarkRunning(evidence, "serving evidence ready; reconciliation pending"));
    WorkerServiceAdmission admission(state);

    EXPECT_TRUE(admission.Check(WorkerAdmissionKind::NORMAL_READ, "Get").IsOk());
    EXPECT_TRUE(admission.Check(WorkerAdmissionKind::NORMAL_WRITE, "Create").IsOk());
    auto migrationRc = admission.Check(WorkerAdmissionKind::MIGRATION_TARGET, "MigrateData");
    EXPECT_EQ(migrationRc.GetCode(), K_NOT_READY);
    EXPECT_NE(migrationRc.GetMsg().find("rejection=RECONCILIATION_INCOMPLETE"), std::string::npos);
    EXPECT_TRUE(admission.Check(WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, "ChangePrimaryCopy").IsOk());
}
...
TEST(WorkerServiceAdmissionTest, AppliesServiceModeMatrix)
{
    ...
    const std::vector<Case> cases = {
        ...
        { WorkerServiceMode::RUNNING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, true, K_OK },
        ...
        { WorkerServiceMode::STARTING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
        ...
        { WorkerServiceMode::JOINING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
        ...
        { WorkerServiceMode::LOCAL_ISOLATED, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
        ...
        { WorkerServiceMode::RECOVERING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
        ...
        { WorkerServiceMode::DRAINING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
        ...
        { WorkerServiceMode::OUT_OF_MEMORY, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_OUT_OF_MEMORY },
        ...
        { WorkerServiceMode::STOPPING, WorkerAdmissionKind::PRIMARY_PROMOTION_RPC, false, K_NOT_READY },
